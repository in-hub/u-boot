...
static struct udevice *ubiblock_find(int ubi_num, const char *volume)
{
	...
	for (blk_first_device(UCLASS_UBI, &dev); dev; blk_next_device(&dev)) {
...
{
	struct ubi_volume_desc *desc = dev_get_uclass_priv(dev);
	...
	while (bytes_left) {
		...
		ret = ubi_read(desc, leb, buffer + bytes_read, offset, to_read);
		if (ret < 0) {
...
int ubiblock_create(int ubi_num, const char *volume)
{
	...
	if (!dev) {
		...
		/* Use default block size to allow using FAT */
		blksz = 512;
		size = vol->reserved_pebs * (leb_size / blksz);
		ret = blk_create_devicef(parent, "ubiblock", volume,
					 UCLASS_UBI, -1, 512, size, &dev);
		if (ret)
			return ret;
		dev_set_uclass_priv(dev, desc);
	}
	blk = dev_get_uclass_plat(dev);
	return blk->devnum;
}
